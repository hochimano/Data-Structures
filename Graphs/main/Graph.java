...
public class Graph<T extends Comparable<T>> {
    ...
    public boolean isEulerianCircuit(){
        ...
        // return false;
            boolean out = true;
        for(Node<T> n : _nodes.values()){
            if(!(n.getEdges().size() == n.getOutEdges().size() && isConnected())){             
                out = false;
            }
        }
        return out;
    ...
    public class eulerianSet{
        ...
        public boolean containsCircuit(){
           Set<eulerianSet> ends = new HashSet<eulerianSet>();
           Queue<eulerianSet> q = new LinkedList<eulerianSet>();
           q.addAll(branches);

           while(!q.isEmpty()){
            eulerianSet current = q.remove();
            q.addAll(current.branches);
            if(current.branches.size() == 0){
                ends.add(current);
            }

            for(eulerianSet s : ends){
                if(s.data.getEdges().containsValue(data)){
                    if(trace.size() == _nodes.size()){
                        return true;
                    }
                }
            }
